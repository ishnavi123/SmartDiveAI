...
export default function App() {
    ...
    try {
      ...
      if (result.name && result.name !== 'None') {
        ...
        if (result.isDangerous) {
          ...
          setWarning(`⚠️ DANGER: ${result.name.toUpperCase()} DETECTED!`);
        }
      } else {
  ...
  const updateHUD = (name: string, currentDepth: number) => {
    const fish = fishList.find(f => f.name.toLowerCase().includes(name.toLowerCase()) || name.toLowerCase().includes(f.name.toLowerCase()));
    setSpecies(name);
    ...
    if (fish) {
      if (!isAlertDismissed) {
        ...
        if (name === "Great White" || name === "Box Jellyfish" || fish.warning.includes('🚨') || fish.warning.includes('💀')) {
          setAlertLevel('danger');
        } else if (fish.warning) {
          setAlertLevel('warning');
        } else {
          setAlertLevel('normal');
        }
